...
calicoctl --allow-version-mismatch patch node ${master} --patch '{"spec": {"bgp": {"asNumber": "65005", "routeReflectorClusterID": "10.0.0.1"}}}'
calicoctl --allow-version-mismatch patch node ${node1} --patch '{"spec": {"bgp": {"asNumber": "65005"}}}'
calicoctl --allow-version-mismatch patch node ${node2} --patch '{"spec": {"bgp": {"asNumber": "65005"}}}'
calicoctl --allow-version-mismatch patch node ${node3} --patch '{"spec": {"bgp": {"asNumber": "65005"}}}'
